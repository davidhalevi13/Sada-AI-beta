...
export const ChatResponse = React.memo(function ChatResponse({
  ...
  const effectiveArtifacts: ChatArtifact[] =
    isStreaming && streamingArtifacts && streamingArtifacts.length > 0
      ? streamingArtifacts
      : persistedArtifacts;
  const visibleArtifacts = effectiveArtifacts.filter(
    (artifact) => artifact.recordId || !isGitHubUrl(artifact.downloadUrl)
  );
  ...
  const shell = (
    <Box style={{ width: '100%' }}>
      ...
      {appliedFilters && (appliedFilters.apps.length > 0 || appliedFilters.kb.length > 0) && (
        <Box className={styles.userMetaStack}>
      ...
      {attachments && attachments.length > 0 && (
        <Flex
          align="center"
          gap="2"
          className={`no-scrollbar ${styles.userMetaStack}`}
          style={{
            overflowX: 'auto',
            overflowY: 'hidden',
          }}
      ...
      <Box className={styles.assistantMessageCard}>
        {/* Tabs */}
        <ResponseTabs
          activeTab={activeTab}
